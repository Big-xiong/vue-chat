  ...
  >
    ...
    >
      <el-form-item :label="$t('dailyWord.form.label.type')" prop="type">
        <el-select
          v-model="params.type"
          clearable
          :placeholder="$t('dailyWord.form.placeholder.type')"
        >
          <el-option
      ...
      <el-form-item :label="$t('dailyWord.form.label.title')" prop="title">
        <el-input
          ...
          :placeholder="$t('dailyWord.form.placeholder.title')"
        />
      </el-form-item>
      <el-form-item
        :label="$t('dailyWord.form.label.content')"
        prop="content"
        class="width-auto"
      >
        <!-- <tinymce v-model="params.content" :height="140" /> -->
    ...
    <span slot="footer">
      ...
      <el-button
        ...
        >{{ $t("action.type.continue") }}</el-button
      >
      <el-button
        type="primary"
        :loading="buttonLoading"
...
<script>
import { dailyWordHandler, getWordTypeList } from "@/api/dailyWord.js";
        ...
        type: [
          {
            required: true,
            trigger: "blur",
            message: this.$t("dailyWord.form.tips.type"),
          },
        ],
        title: [
          {
            required: true,
            trigger: "blur",
            message: this.$t("dailyWord.form.tips.title"),
          },
        ],
        content: [
          {
            required: true,
            trigger: "blur",
            message: this.$t("dailyWord.form.tips.content"),
          },
        ],
      },
      wordTypeList: [],
    ...
    handleAddContinue() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          const url = this.type == "add" ? "/api/addWord" : "/api/editWord";
          const { id: word_id, type: cate_id, title: title } = this.params;
          dailyWordHandler(url, {
            cate_id,
            title,
            word: text,
            word_id,
