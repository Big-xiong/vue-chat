<template>
	<div class="fixed">
		<div class="title">
			{{ $t('connectServer.halfFixed.text[0]') }}
		</div>
		<p class="tips">{{ $t('connectServer.halfFixed.text[7]') }}</p>
		<el-button class="mt4" type="text" icon="el-icon-info" @click="courseVisible=true">{{ $t('connectServer.halfFixed.text[1]') }}</el-button>
		<div class="flex-aic">
			<el-select v-model="domainList" class="w390" clearable size="medium" :placeholder="$t('connectServer.halfFixed.placeholder.platform')">
				<el-option v-for="platform in platformList" :key="platform.label" :label="platform.label" :value="platform.children" />
			</el-select>
			<div class="ml10">{{ $t('connectServer.halfFixed.text[2]') }}</div>
		</div>
		<div class="flex-aic mt20">
			<el-select v-model="groupId" class="w190 mr10" clearable size="medium" value-key="group_id" :placeholder="$t('connectServer.halfFixed.placeholder.group')" @change="genServerList">
				<el-option v-for="group in groupList" :key="group.group_id" :label="group.group_name" :value="group" />
			</el-select>
			<el-select v-model="server" class="w190" size="medium" clearable value-key="kefu_code" :placeholder="$t('connectServer.halfFixed.placeholder.server')">
				<el-option v-for="server in serverList" :key="server.kefu_code" :label="server.kefu_name" :value="server" />
			</el-select>
			<div class="ml10">{{ $t('connectServer.halfFixed.text[3]') }}</div>
		</div>
		<!-- 自定义参数 -->
		<div class="title mt40">{{ $t("connectServer.auto.text[2]") }}</div>
		...
		<el-button v-waves class="mt40 mb44" type="primary" @click="genSeverLink"><i class="material-icons-round fs-14 mr6">insights</i>{{ $t('connectServer.halfFixed.text[5]') }}</el-button>
		<el-table class="mt16" :data="serverLinkList" stripe>
			<el-table-column prop="name" :label="$t('connectServer.halfFixed.table.name')" width="150" />
			<el-table-column prop="link" :label="$t('connectServer.halfFixed.table.link')" />
...
export default {
	...
	methods: {
		...
		genSeverLink () {
			...
			if (!domainList) {
				return this.$message({
					message: this.$t('connectServer.halfFixed.tips.platform'),
					type: 'error'
				})
			}
			if (!groupId) {
				return this.$message({
					message: this.$t('connectServer.halfFixed.tips.group'),
					type: 'error'
				})
			}
			...
			if (firstDomain) {
				serverList.forEach(server => {
					serverLinkList.push({
						...
						link: `${process.env.VUE_APP_LINK}/chat/client/seller_${firstDomain.seller_code}/server_${server.kefu_code}/domain_${firstDomain.domain_name}${params}/source_1`
					})
				})
			}
